...
function showListHelp(): void {
  console.log(`ec post list - List posts

Usage: ec post list [options]

Options:
  --limit <n>       Maximum number of posts (default: 50, max: 100)
  --tag <tag>       Filter by tag slug
  --status <status> Filter by status: draft, published, all (default: published)
  --type <type>     Filter by type: article, link, note
  --json            Output as JSON
...
Examples:
  ...
  ec post list --type article --status all
`);
}
...
export async function list(args: string[], globalOptions: GlobalOptions): Promise<void> {
  ...
  const result = await client.listPosts({
    limit: options.limit,
    tag: options.tag,
    status: options.status || "all", // Default to all for CLI
    type: options.type,
  });
